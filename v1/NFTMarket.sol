...
contract NFTMarket is ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        // Percentage Logic, considering 1%, solidity dont understand decimal for 0.5% ==> 5/1000
        uint256 cutPrice = (price * 1) / 100;
        uint256 newPrice = price - cutPrice;

        idToMarketItem[itemId].seller.transfer(newPrice);
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        idToMarketItem[itemId].owner = payable(msg.sender);
        idToMarketItem[itemId].sold = true;
        _itemsSold.increment();
        payable(owner).transfer(cutPrice);
    }

        /* Creates the sale of a marketplace item */
    /* Transfers ownership of the item, as well as funds between parties */
    function buyNftForOthers(address nftContract, uint256 itemId, address buyer)
        public
        payable
        nonReentrant
    {
    ...
    function fetchAllMarketNFTs() public view returns (MarketItem[] memory) {
